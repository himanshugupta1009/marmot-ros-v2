...
class StateUpdater:
    ...
    def store_people_pose_array(self, msg):
        current_time = rospy.Time.now()
        for i, ped_id in enumerate(msg.ids):
            if ped_id <= 0 or ped_id > self.num_pedestrians:
                rospy.logwarn("Received ped_id={} outside expected range 1 - {}".format(ped_id, self.num_pedestrians))
                continue
            index = ped_id - 1  # convert 1-based ID to 0-based index
            # Construct a PoseStamped-like message
            pose_msg = PoseStamped()
            pose_msg.header = msg.header
            pose_msg.pose = msg.poses[i]
            self.current_ped_msgs[index] = pose_msg
            if self.record_hist:
                self.hist_ped_msgs[index].append(pose_msg)
    ...
    def __init__(self, num_pedestrians):
        try:
            ...
            if USE_DUMMY_DATA:
                rospy.Subscriber(
                    "/car/dummy/people_poses",
                    PeoplePoseArray,
                    callback=self.store_people_pose_array,
                    queue_size=1
                )
            else:
                ...
                self.last_store_time = rospy.Time.now()
...
def save_s_hist(self):
    ...
    # Get the starting timestamp for the directory name
    ...
    # Define file paths for vehicle and pedestrian histories
    ...
    # Save pedestrian histories
    for i, ped_file in enumerate(ped_filename):
        with open(ped_file, 'w') as ped_file:
    ...
    print("Save complete for vehicle and pedestrian histories in {}".format(dir_name))
...
if __name__ == '__main__':
    try:
        ...
        # setting the flag via a ROS launch file
        # TO RUN WITH FLAG TRUE, use the cmd below
        # roslaunch state_updater_pkg state_updater.launch use_dummy_data:=true
        USE_DUMMY_DATA = rospy.get_param("~use_dummy_data", False)

        if USE_DUMMY_DATA:
            rospy.loginfo("Using dummy data!")
        else:
            rospy.loginfo("Using VICON data!")

        # Get the number of pedestrians from a ROS parameter, defaulting to 4 if not set. To control this value, edit state_updater.launch file
        num_pedestrians = rospy.get_param("~num_pedestrians", 4)
        
        # Initialize the StateUpdater with the specified number of pedestrians
